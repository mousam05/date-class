...
public:
	...
	~DateFormat();
	//THE FOLLOWING FUNCTIONS HAVE BEEN ADDED BY ME, NOT GIVEN IN THE ORIGINAL ASSIGNMENT
	DateFormat& operator=(const DateFormat &); //copy assignment operator, required in Date::setFormat()

	//getter functions for private data members, required for accessing in Date class while taking I/0
	char* getdateFormat();
	char* getmonthFormat();
	char* getyearFormat();
